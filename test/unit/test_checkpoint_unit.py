...
@pytest.fixture
def docstring_map():
    return ExampleDataclass(cheese="asdf", crackers=5)
...
def test_get_all_data(docstring_map, temp_file):
    ...
    assert all(isinstance(x, ExampleDataclass) for x in checkpoint.get_all())
    assert checkpoint.get_all()[0].crackers == 5
